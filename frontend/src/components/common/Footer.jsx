...
const Footer = memo(() => {
  ...
  const handleSocialPointerMove = useCallback((event, index) => {
    ...
    setHoveredSocial(prev => (prev === index ? prev : index));
        ...
        <div className="py-8 border-b border-gray-700/50">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {features.map((feature, index) => (
              <div
                key={index}
                className={`relative glass p-5 rounded-xl flex items-center justify-center space-x-3 text-center overflow-hidden transition-all duration-500 ${enableAnimations ? 'hover:bg-white/10' : ''}`}
                style={enableAnimations ? { animationDelay: `${index * 120}ms` } : undefined}
              >
                <div
                  className={`absolute inset-[1px] rounded-[0.65rem] bg-gradient-to-r from-blue-500/0 via-blue-500/15 to-purple-500/0 opacity-0 ${shimmerClass}`}
                  style={{ opacity: enableAnimations ? 1 : 0.4 }}
                  aria-hidden="true"
                ></div>
                <feature.icon className={`relative z-10 h-6 w-6 text-blue-400 ${floatClass}`} aria-hidden="true" />
                <span className="relative z-10 text-gray-300 font-medium">{feature.text}</span>
              </div>
            ))}
          </div>
        </div>
                  ...
                  {socialLinks.map((social, index) => (
                    ...
                    >
                      <div
                        ...
                        style={{
                          opacity: hoveredSocial === index ? 1 : enableAnimations ? 0 : 0.25,
                      ...
                      <div className="relative z-10 mt-2 text-[0.65rem] text-gray-400 group-hover:text-gray-200">
                        {social.followers}
                      </div>
                    </a>
                  ))}
